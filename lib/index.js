...
function getUrl (fullPath, opts) {
  opts = opts || {}
  let parsedPath = url.parse(fullPath)
  let file = parsedPath.pathname
  ...
  if (isCss(file)) {
    content = rework(content.toString()).use(reworkUrl(rewriteUrl({filename: file}))).toString()
  }
  const u = urify(file, content)
  writeAsset(u, content)
  return `${BASE}${u}${parsedPath.search || ''}${parsedPath.hash || ''}`
}
